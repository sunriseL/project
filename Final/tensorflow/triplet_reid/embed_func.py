...
def embed(image,dataset='data/query.csv',image_root='',experiment_root='/home/sunrise/experiments/my_experiment',filename='query.h5',file_index='triplet_reid.'):
    # Verify that parameters are set correctly.
    #args = parser.parse_args()
    #print(args)
    args = Namespace(aggregator='mean', batch_size=256, checkpoint='/home/sunrise/experiments/my_experiment/checkpoint-25000', crop_augment=None, dataset=dataset, experiment_root=experiment_root, filename=filename, flip_augment=True, image_root=image_root, loading_threads=8, quiet=False)
    print(args)
    ...
    _, data_fids,_,_,_,_,_ = common.load_dataset(args.dataset, args.image_root)
    print(data_fids)
    ...
    dataset = dataset.map(
        lambda fid: common.byte_to_image(
            image,fid=tf.constant('1.jpg'),pid=tf.constant('1'),
            image_size=pre_crop_size if args.crop_augment else net_input_size))
    print(dataset)
    ...
    model = import_module(file_index + 'nets.' + args.model_name)
    head = import_module(file_index + 'heads.' + args.head_name)
...
def embed_single_image(image,dataset='data/query.csv',image_root='',experiment_root='/home/sunrise/experiments/my_experiment',filename='query.h5',file_index='triplet_reid.'):
    # Verify that parameters are set correctly.
    #args = parser.parse_args()
    #print(args)
    args = Namespace(aggregator='mean', batch_size=256, checkpoint='/home/sunrise/experiments/my_experiment/checkpoint-25000', crop_augment=None, dataset=dataset, experiment_root=experiment_root, filename=filename, flip_augment=True, image_root=image_root, loading_threads=8, quiet=False)
