    ...
    with tf.Session() as sess:
        for start_idx in count(step=args.batch_size):
            ...
            for i in range(len(distances)):
                ...
                origin_image = query_fids[i]
                out_image = gallery_fids[np.argsort(distances[i])[0]]
                print(query_fids[i],gallery_fids[np.argsort(distances[i])[0]])
                img1 = cv2.imread('../' + origin_image)
                img2 = cv2.imread('../' + out_image) 
